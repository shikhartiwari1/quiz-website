...
const text = `1. Consumer behaviour primarily helps marketers understand how buyers _________ among the competing brands.
...
for (let line of lines) {
    line = line.trim();
    if (!line) continue;
    if (line.match(/^\d+–\d+$/)) continue; // skip 11-20 headers
    ...
    if (line.match(/^\d+\.\s/)) {
        ...
        if (currentQ) currentQ.answer = line.replace('Answer: ', '');
    }
}
if (currentQ) questions.push(currentQ);

const jsonStr = JSON.stringify(questions, null, 16);
const formattedArray = '[\n' + jsonStr.substring(jsonStr.indexOf('[') + 1, jsonStr.lastIndexOf(']')) + '\n            ]';

const indexHtmlPath = 'c:\\Users\\SHIKHAR TIWARI\\Desktop\\quiz\\index.html';
const htmlContent = fs.readFileSync(indexHtmlPath, 'utf8');

const regex = /(const originalQuestions =\s*)\[([\s\S]*?)\];/g;
const newHtml = htmlContent.replace(regex, `$1${formattedArray};`);

fs.writeFileSync(indexHtmlPath, newHtml, 'utf8');
console.log('Replaced successfully! Array size:', questions.length);
